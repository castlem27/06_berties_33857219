...
API
The Books API provides JSON access to the books stored in the application’s MySQL database through a single endpoint at /api/books. By default, it returns a full list of all books, but it also supports optional URL query parameters for more advanced searches. The search parameter allows users to filter books by name using a keyword match, while minprice and maxprice can be used individually or together to limit results to a specific price range. Additionally, the sort parameter can be used to order the results either alphabetically by book name or numerically by price. All responses are returned in JSON format, and the API is designed to provide flexible filtering and sorting through one route without requiring multiple endpoints.